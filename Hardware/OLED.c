...
/**
  * 函    数：交换两个数掩码
  * 参    数：num_high高位数，low_high低位数
  * 参    数：第n位，掩码从高位数的0-n，取到低位数的n-8
  * 返 回 值：取好的掩码
  */
uint8_t OLED_SwapMaskBits(uint8_t num_high, uint8_t num_low,uint8_t n)
{
	uint8_t high_mask;
	uint8_t low_mask;
	...
	high_mask = 0xFF << (8 - n);
	low_mask = 0xFF >> n;
	high_bits = num_high & high_mask;
	low_bits = num_low & low_mask;
...
void OLED_DisplayBuffMove(uint8_t Direct)
{
	...
	for(Y=0;Y<8;Y++)
	{
		if(!Direct)
		{
			for(X=0;X<127;X++)
			{
				OLED_DisplayBuf[Y][X] = OLED_DisplayBuf[Y][X+1];
				if(X==126)OLED_DisplayBuf[Y][X+1]=0;
			}
		...
		}else
		{
			...
			{
				OLED_DisplayBuf[Y][X] = OLED_DisplayBuf[Y][X-1];
				if(X==1)OLED_DisplayBuf[Y][X-1]=0;
			}
		}
...
void OLED_DisplayBuffMoveArea(uint8_t PosX, uint8_t PosY, uint8_t EndX, uint8_t EndY, uint8_t Direct) 
{
    ...
    uint8_t end_byte = EndY / 8;

    // 遍历每一行
    for (Y = start_byte; Y <= end_byte; Y++) {
        // 计算当前行的位偏移
        uint8_t bit_offset_start = (Y == start_byte) ? (PosY % 8) : 0;
        uint8_t bit_offset_end = (Y == end_byte) ? (EndY % 8) : 7;

        if (!Direct) {
            // 左移
            for (X = PosX; X <= EndX; X++) {
                uint8_t current_byte = OLED_DisplayBuf[Y][X];
                uint8_t next_byte = (X == EndX) ? 0 : OLED_DisplayBuf[Y][X + 1];

                // 处理位跨越字节的情况
                if (bit_offset_start > 0 || bit_offset_end < 7) {
                    uint8_t n = bit_offset_end - bit_offset_start + 1;
                    OLED_DisplayBuf[Y][X] = OLED_SwapMaskBits(current_byte, next_byte, n);
                } else {
                    OLED_DisplayBuf[Y][X] = next_byte;
                }
            }
        } else {
            // 右移
            for (X = EndX; X >= PosX; X--) {
                uint8_t current_byte = OLED_DisplayBuf[Y][X];
                uint8_t prev_byte = (X == PosX) ? 0 : OLED_DisplayBuf[Y][X - 1];

                // 处理位跨越字节的情况
                if (bit_offset_start > 0 || bit_offset_end < 7) {
                    uint8_t n = bit_offset_end - bit_offset_start + 1;
                    OLED_DisplayBuf[Y][X] = OLED_SwapMaskBits(current_byte, prev_byte, n);
                } else {
                    OLED_DisplayBuf[Y][X] = prev_byte;
                }
            }
        }
    }
...
//}
{
	...
	while (String[i] != '\0')	//遍历字符串
	{
		...
		if(XOffset+6 > 127 && !(CharLength == 1 && SingleChar[0]=='\n'))         //    缺字显示
		{
			XOffset = 0;
			Y += 12;
		}
...
/**
  * 函    数：OLED设置对比度/亮度
  * 参    数：contrast 对比度值，范围：0x00~0xFF
  * 返 回 值：无
  * 说    明：值越大，显示越亮
  */
void OLED_SetContrast(uint8_t contrast)
{
    ...
    OLED_WriteCommand(contrast); // 设置对比度值(0-255)
}

/**
  * 函    数：OLED设置亮度级别
  * 参    数：level 亮度级别，范围：0~100
  * 返 回 值：无
  * 说    明：将0-100线性映射到0x00-0xFF
  */
void OLED_SetBrightness(uint8_t level)
{
    // 将0-100的亮度级别映射到0x00-0xFF的对比度值
    uint8_t contrast;
    if(level > 100) level = 100;
    
    // 线性映射
    contrast = (uint8_t)((level * 255) / 100);
    
    // 非线性映射（更符合人眼感知）
    // contrast = (uint8_t)(pow((float)level/100.0f, 1.5f) * 255);
